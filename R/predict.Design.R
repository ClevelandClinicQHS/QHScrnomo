...
##' @param se.fit return standard error
##' @param conf.int specify conf.int as a positive fraction to obtain upper and 
##' lower confidence intervals (e.g., `conf.int=0.95`).
##' @param conf.type specifies the type of confidence interval.
##' @param incl.non.slopes set to `TRUE` to include multiple intercepts, default is `NULL`
##' @param non.slopes a vector of multiple intercepts
##' @param kint a single integer specifying the number of the intercept to use 
##' in multiple-intercept models. 
##' @param na.action Function to handle missing values in `newdata`. 
##' @param expand.na set to `FALSE` to keep the naresid from having any effect
##' @param center.terms set to `FALSE` to suppress subtracting adjust-to values 
...
##' @seealso  \code{\link[rms]{predictrms}}, 
##'
predictDesign <-
    function(
        fit, newdata = NULL,
